...
class Bot extends Slackbots {
	...
	registerHandler(handler) {
		let groups = handler.groups;
		for (const group of groups) {
			if (group === 'channel' || group === 'direct' || group === 'admin') {
				this.cmds[group].push(handler);
			} else {
	...
	_handleMessage(message, target) {
		...
		for (const cmd of CMDS) {
			...
			if (result !== null) {
				matchResult = result;
				handler = cmd.handler;
				message.matchResult = matchResult;
			}
		}

		if (handler !== null) {
			handler.call(this, message, (response, channel, params) => {
				if (params) {
